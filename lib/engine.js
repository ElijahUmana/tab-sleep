...
export class TabSleepEngine {
  ...
  async resolvePreviewTab(token, senderTab = null) {
    ...
    for (const tabId of new Set(candidateIds.filter(Number.isInteger))) {
      ...
      if (tab && this.previewToken(tab.url ?? tab.pendingUrl) === token) return tab;
    }
    return null;
  }
  ...
  async performScan(ignoreIdle) {
    ...
    const tabs = await this.chrome.tabs.query({});
    const state = await this.readState();
    // Every selected tab in an open window is visible by definition — Chrome
    // reports each window's state directly. This is authoritative for tiled /
    // occluded windows where paint signals go dark.
    let windowsById = null;
    try {
      const allWindows = await this.chrome.windows.getAll();
      if (Array.isArray(allWindows)) {
        windowsById = new Map(allWindows.map((window) => [window.id, window]));
      }
    } catch {}
    ...
    for (const tab of candidates) {
      try { if (await this.freeze(tab, settings, ignoreIdle, forceVisible)) frozen.push(tab.id); }
    ...
    try {
      const results = await this.chrome.scripting.executeScript({
        ...
        func: (sentinel, originalUrl) => {
          ...
          for (const node of clone.querySelectorAll("script, noscript, iframe, object, embed, audio, video")) {
            node.remove();
          }
  ...
  async freeze(tab, settings, ignoreIdle, forceVisible = null) {
    await this.stateQueue;
    ...
    if (getAwakeTabBlockReason(current, settings, state, this.clock(), { ignoreIdle, forceVisible }) !== null) return false;
    const key = String(tab.id);
  ...
  async rollbackFailedFreeze(originalTab, key, token) {
    ...
    if (current && this.previewToken(current.url) === token) {
  ...
  async status() {
    ...
    return { settings, metrics: normalizeMetrics(local[METRICS_KEY]), summary: summarizeTabs(tabs, settings, state, this.clock(), this.previewUrlPrefix), currentTabProtected: current ? Boolean(state.protectedTabIds[String(current.id)]) : false, currentTabState: frozen?.status ?? "awake", currentTabVerifiedSleeping: Boolean(frozen?.verifiedSleeping), nextScanAt: (await this.chrome.alarms.get(ALARM_NAME))?.scheduledTime ?? null };
  }
