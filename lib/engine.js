...
export class TabSleepEngine {
  constructor(chromeApi, clock = () => Date.now(), tokenFactory = () => crypto.randomUUID(), options = {}) {
    ...
    this.previewIndexQueue = Promise.resolve();
    this.pendingPreviews = new Map();
    this.inFlightPreviewTokens = new Set();
    ...
    this.previewStore.protectedTokensProvider = () => this.protectedPreviewTokens();
  ...
  async handlePreviewReady(payload, tab) {
    ...
    this.inFlightPreviewTokens.delete(payload.token);
    await this.mutate((state) => {
      const frozen = state.frozenTabs[String(tabId)];
  ...
  async handlePreviewFailed(payload, tab) {
    if (!payload?.token) return { parked: false };
    const actualTab = await this.resolvePreviewTab(payload.token, tab);
    if (!actualTab) return { parked: false };
    const key = String(actualTab.id);
    await this.mutate((state) => {
      const frozen = state.frozenTabs[key];
      if (frozen?.token !== payload.token) return;
      frozen.status = "sleeping";
      frozen.verifiedSleeping = false;
      frozen.previewError = String(payload.error ?? "Frozen visual is unavailable");
    });
    // Rendering failure must never wake or navigate a sleeping tab. The parked
    // page remains in place and its trusted click/Enter/Space wake surface is
    // still available even when the visual itself cannot be recovered.
    return { parked: true };
  }
  ...
  async reconcile() {
    ...
    const protectedTokens = await this.protectedPreviewTokens();
    await this.previewStore.reconcile(protectedTokens);
    const staleTokens = [];
    ...
    if (staleTokens.length > 0) {
      await this.chrome.storage.local.remove(staleTokens.map((token) => `${PREVIEW_KEY_PREFIX}${token}`));
    }
  }
  ...
  async protectedPreviewTokens(tabs = null) {
    const [openTabs, stored] = await Promise.all([
      tabs ? Promise.resolve(tabs) : this.chrome.tabs.query({}),
      this.chrome.storage.local.get(WAKE_TX_KEY)
    ]);
    const protectedTokens = new Set(this.inFlightPreviewTokens);
    for (const tab of openTabs) {
      const token = this.previewToken(tab.pendingUrl ?? tab.url);
      if (token) protectedTokens.add(token);
    }
    for (const transaction of Object.values(stored[WAKE_TX_KEY] ?? {})) {
      if (typeof transaction?.token === "string" && transaction.token) protectedTokens.add(transaction.token);
    }
  ...
  async captureFullPage(tab, token) {
    ...
    const documentTiles = capture.tiles ?? [{ imageDataUrl: capture.imageDataUrl, index: 0, y: 0, width: capture.width, height: capture.height }];
    const documentImages = documentTiles.map((tile) => ({
      ...
      yOffset: tile.y,
      width: tile.width ?? capture.width,
      height: tile.height
    }));
    const nestedImages = (capture.nestedRegions ?? []).flatMap((region) => region.tiles.map((tile) => ({
      bytes: dataUrlToBytes(tile.imageDataUrl),
      mime: "image/webp",
      kind: "nested",
    ...
    const images = [...documentImages, ...nestedImages];
    const nestedRegions = (capture.nestedRegions ?? []).map((region) => ({
      index: region.index,
      x: region.x,
      y: region.y,
      ...
      originalScrollTop: region.originalScrollTop
    }));
    ...
    // Shape consumed by freeze()/restorePreview(): keeps only bounded geometry
    // and lifecycle metadata; binary capture buffers remain in IndexedDB.
    return {
      token,
      originalUrl: tab.url,
      title: tab.title ?? "Sleeping tab",
  ...
  async freeze(tab, settings, ignoreIdle) {
    ...
    // Every freeze acquires a fresh ENTIRE scrollable-page capture. A cached
    // visible viewport is never promoted into a sleeping visual: it is neither
    // the full document nor enough for nested application scrollers.
    const token = this.tokenFactory();
    const previewKey = `${current.id}:${token}`;
    this.inFlightPreviewTokens.add(token);
    try {
      let record = await this.captureFullPage(current, token);
      if (!record) record = await this.captureDomSnapshot(current, token);
      const hasVisual = Boolean(record && (record.hasTiles || record.imageCount > 0 || typeof record.html === "string" || record.images?.length > 0));
      if (!hasVisual) return false;
      record.frozenAt = this.clock();
      await this.previewStore.markFrozen(token, record.frozenAt);
      await this.mutate((next) => {
        next.captures[key] = { token, url: current.url, capturedAt: record.capturedAt, hasImage: hasVisual };
      ...
      this.inFlightPreviewTokens.delete(token);
  ...
  async startWake(token, senderTab, { navigate = false } = {}) {
    ...
    const metadata = await this.previewStore.getIndexedMetadata(token);
    const originalUrl = isPreviewableUrl(metadata?.originalUrl) ? metadata.originalUrl : frozen.originalUrl;
    if (!isPreviewableUrl(originalUrl)) throw new Error("Original page URL is unavailable");
    const proposed = { token, tabId, originalUrl, title: metadata?.title ?? frozen.title ?? null, startedAt: this.clock() };
    const { transaction: tx, claimed } = await this.mutateWakeTxs((transactions) => {
      const pending = transactions[key];
      if (pending?.token === token) return { transaction: pending, claimed: false };
      transactions[key] = proposed;
      return { transaction: proposed, claimed: true };
    });
    if (claimed) {
      await this.mutate((next) => {
    ...
    if (navigate && (actualTab.pendingUrl ?? actualTab.url) !== tx.originalUrl) {
      await this.chrome.tabs.update(tabId, { url: tx.originalUrl });
    }
  ...
  mutateWakeTxs(mutator) {
    const op = this.wakeTxQueue.then(async () => {
      ...
      const result = await mutator(transactions);
  ...
  async restorePreview(tab, token) {
    let metadata = await this.previewStore.getIndexedMetadata(token);
    if (!metadata) {
      // Legacy 4.x payloads may total multiple gigabytes. Startup and tab-event
      // reconciliation must never decode them. The small preview index is enough
      // to re-anchor lifecycle state; PREVIEW_GET_RECORD migrates this one token
      // only when its frozen page actually renders.
      const indexed = await this.chrome.storage.local.get(PREVIEW_INDEX_KEY);
      const legacy = indexed[PREVIEW_INDEX_KEY]?.[token];
      if (isPreviewableUrl(legacy?.originalUrl)) {
        metadata = {
          token,
          originalUrl: legacy.originalUrl,
          title: legacy.title ?? tab.title ?? "Sleeping tab",
          capturedAt: legacy.updatedAt ?? this.clock(),
          frozenAt: legacy.updatedAt ?? null,
          legacyIndexed: true
        };
      }
    }
    const hasVisual = Boolean(metadata && (metadata.legacyIndexed || metadata.images?.length > 0 || typeof metadata.html === "string"));
    if (!metadata || !isPreviewableUrl(metadata.originalUrl) || !hasVisual) {
      // Missing visuals remain parked. Selection alone must never navigate the
      // tab or recreate its live renderer; only an explicit trusted wake may do
      // that. Preserve the indexed URL so the parked error surface can wake.
      const indexed = await this.chrome.storage.local.get(PREVIEW_INDEX_KEY);
      const fallback = indexed[PREVIEW_INDEX_KEY]?.[token];
      if (!isPreviewableUrl(fallback?.originalUrl)) return;
      metadata = {
        token,
        originalUrl: fallback.originalUrl,
        title: fallback.title ?? tab.title ?? "Sleeping tab",
        capturedAt: fallback.updatedAt ?? this.clock(),
        frozenAt: fallback.updatedAt ?? null,
        missingVisual: true
      };
    }
    const key = String(tab.id);
    await this.mutate((state) => {
      ...
      state.captures[key] = { token, url: metadata.originalUrl, capturedAt: metadata.capturedAt, hasImage: hasVisual && !metadata.missingVisual };
  ...
  mutateIndex(mutator) { const op = this.previewIndexQueue.then(async () => { const stored = await this.chrome.storage.local.get(PREVIEW_INDEX_KEY); const index = stored[PREVIEW_INDEX_KEY] ?? {}; await mutator(index); await this.chrome.storage.local.set({ [PREVIEW_INDEX_KEY]: index }); return clone(index); }); this.previewIndexQueue = op.catch(() => {}); return op; }
}
