...
export function summarizeTabs(tabs, settings, state, now, previewUrlPrefix, forceVisible = new Set()) {
  ...
  for (const tab of tabs) {
    ...
    const reason = getAwakeTabBlockReason(tab, settings, state, now, { forceVisible });
    if (reason === null) summary.due++;
    if (reason === "stale-snapshot") summary.waitingForPreview++;
    if (getAwakeTabBlockReason(tab, settings, state, now, { ignoreIdle: true, forceVisible }) === null) summary.eligibleNow++;
  }
  return summary;
}
