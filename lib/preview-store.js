...
// A versioned schema plus migrateLegacyRecords() carries 4.x storage.local
// rows into this store; restart reconciliation repairs orphans afterwards.
...
export class PreviewStore {
  ...
  async hasPreview(token) {
    return (await this.getMetadata(token)) !== null;
  }
  ...
  /**
   * One-shot migration from chrome.storage.local 4.x records. Idempotent:
   * reads every remaining "preview:<token>" key, converts it into the IDB
   * stores, verifies counts match, then removes only successfully migrated
   * keys. Safe to interrupt — leftover keys simply migrate again next start.
   */
  async migrateLegacyRecords(storageArea) {
    const stored = await storageArea.get(null);
    const legacyKeys = Object.keys(stored ?? {}).filter((key) => key.startsWith(PREVIEW_KEY_PREFIX));
    if (legacyKeys.length === 0) return { migrated: 0, failed: [] };
    const failed = [];
    for (const key of legacyKeys) {
      const record = stored[key];
      if (!isLegacyRecord(record)) {
        // Unrecognizable junk under our prefix cannot be converted; keep the
        // key rather than silently destroying unknown user data.
        failed.push(key);
        continue;
      }
      try {
        ...
        const verified = await this.getMetadata(record.token);
        if (!verified) throw new Error(`Verification read after migration failed for ${record.token}`);
        await storageArea.remove(key);
      } catch {
        failed.push(key);
      }
    }
    return { migrated: legacyKeys.length - failed.length, failed };
  }
