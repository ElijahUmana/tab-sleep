...
export class PreviewStore {
  constructor(options = {}) {
    ...
    this.protectedTokensProvider = options.protectedTokensProvider ?? (() => new Set());
    this.dbPromise = null;
  ...
  async performSavePreview(preview) {
    ...
    const hasDomVisual = typeof preview.html === "string" && preview.html.length >= 20;
    if (images.length === 0 && !hasDomVisual) {
      throw new Error(`Preview ${preview.token} has no durable visual`);
    }
    ...
    for (const image of images) {
      ...
      if (!image.bytes && typeof image.contentHash === "string") {
        ...
        referenceImages.push({
          ...
          mime: image.mime,
          kind: image.kind === "tile" || image.kind === "nested" ? image.kind : "viewport",
          regionIndex: Number.isInteger(image.regionIndex) ? image.regionIndex : null,
    ...
    const metadata = {
      ...
      nestedRegions: Array.isArray(preview.nestedRegions) ? structuredClone(preview.nestedRegions) : [],
      images: [...preparedImages, ...referenceImages].map(({ contentHash, mime, kind, regionIndex, tileIndex, yOffset, width, height }) =>
        ({ contentHash, mime, kind, regionIndex, tileIndex, yOffset, width, height })),
  ...
  markFrozen(token, frozenAt) {
    if (typeof token !== "string" || !token || !Number.isFinite(frozenAt)) {
      return Promise.reject(new Error("markFrozen requires a token and timestamp"));
    }
    return this.enqueue(async () => {
      ...
      try {
        ...
        const metadata = await requestAsPromise(store.get(token));
        if (!metadata) throw new Error(`Preview ${token} is missing before freeze commit`);
        store.put({ ...metadata, frozenAt });
        await transactionDone(transaction);
      } catch (error) {
        try { transaction.abort(); } catch {}
        throw error;
  ...
  deletePreviews(tokens) {
    ...
    return this.enqueue(() => this.deletePreviewsInternal(list));
  }

  // ---- budgets -----------------------------------------------------------------

  async protectedTokens() {
    const tokens = await this.protectedTokensProvider();
    return tokens instanceof Set ? tokens : new Set(tokens ?? []);
  }
  ...
  async enforceBudgets(incomingMetadata) {
    ...
    const perTabBytes = sameTab.reduce((total, record) => total + (record.totalImageBytes ?? 0), 0) + incomingBytes;
    const profileBytes = all.reduce((total, record) => total + (record.totalImageBytes ?? 0), 0) + incomingBytes;
    if (perTabBytes <= this.perTabLimitBytes && profileBytes <= this.profileBudgetBytes) return;

    const protectedTokens = await this.protectedTokens();
    const victims = new Map();
    let remainingPerTab = perTabBytes;
    if (remainingPerTab > this.perTabLimitBytes) {
      sameTab.sort((a, b) => (a.capturedAt ?? 0) - (b.capturedAt ?? 0));
      for (const stale of sameTab) {
        if (remainingPerTab <= this.perTabLimitBytes) break;
        if (protectedTokens.has(stale.token)) continue;
        victims.set(stale.token, stale);
        remainingPerTab -= stale.totalImageBytes ?? 0;
      }
      if (remainingPerTab > this.perTabLimitBytes) {
    ...
    let remainingProfile = profileBytes - [...victims.values()].reduce((total, record) => total + (record.totalImageBytes ?? 0), 0);
    if (remainingProfile > this.profileBudgetBytes) {
      const lru = all.sort((a, b) => (a.capturedAt ?? 0) - (b.capturedAt ?? 0));
      for (const stale of lru) {
        if (remainingProfile <= this.profileBudgetBytes) break;
        if (protectedTokens.has(stale.token) || victims.has(stale.token)) continue;
        victims.set(stale.token, stale);
        remainingProfile -= stale.totalImageBytes ?? 0;
      }
      if (remainingProfile > this.profileBudgetBytes) {
        throw new Error(`Profile preview budget of ${this.profileBudgetBytes} bytes exceeded even after LRU cleanup`);
      }
    }
    await this.deletePreviewsInternal([...victims.keys()]);
  }
  ...
  reconcile(reachableTokens) {
    ...
    return this.enqueue(async () => {
      ...
      try {
        const metadataStore = transaction.objectStore(METADATA_STORE);
        const blobStore = transaction.objectStore(BLOB_STORE);
        const all = await requestAsPromise(metadataStore.getAll());
        const blobRows = await requestAsPromise(blobStore.getAll());
        const availableHashes = new Set(blobRows.filter((row) => row?.blob).map((row) => row.contentHash));
        const keptRecords = [];
        const dropped = [];
        for (const record of all) {
          const broken = (record.images ?? []).some((image) => !availableHashes.has(image.contentHash));
          if (broken || !reachable.has(record.token)) {
            metadataStore.delete(record.token);
            dropped.push(record.token);
          } else {
            keptRecords.push(record);
          }
        }
        const referenced = new Set(keptRecords.flatMap((record) => (record.images ?? []).map((image) => image.contentHash)));
        ...
        await transactionDone(transaction);
        return { kept: keptRecords.length, droppedOrphans: dropped.length, prunedBlobs: orphans.length };
      } catch (error) {
        try { transaction.abort(); } catch {}
        throw error;
      }
